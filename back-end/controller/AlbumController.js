...
class AlbumController {
    ...
    try {
      ...
      if (req.files && req.files.tracks) {
        ...
        albumData.tracks = req.files.tracks.map((file, index) => ({
          ...
          url: `${baseUrl}/assets/music/${file.filename}`,
  ...
  async downloadTrack(req, res) {
    try {
      ...
      // >>> CÓDIGO DE DATA LIFE CYCLE <<<
      if (track.isArchived) {
      console.log(`[Lifecycle] Restaurando pista archivada: ${track.title}`);
      // Usamos await para asegurarnos de que el archivo se escriba en disco antes de seguir
      await lifecycleService.restoreTrack(album._id, track.id); 
      }

      // Actualizamos la fecha de acceso para el ciclo de vida
      track.lastAccessed = new Date();
      await album.save(); 
      // >>> FIN DEL BLOQUE DE DATA LIFE CYCLE <<<
      ...
      // >>> REEMPLAZA TU BLOQUE DE DATA LIFE CYCLE POR ESTE <<<
      if (!fs.existsSync(audioPath)) {
          console.log(`[ALERTA] El archivo ${filename} no está. Intentando forzar restauración...`);
          
          const lifecycleService = require('../services/LifeCycleService');
          // Llamamos a restore directamente
          await lifecycleService.restoreTrack(album._id, track.id);
          
          // Re-comprobar tras restaurar
          if (fs.existsSync(audioPath)) {
              console.log("¡ÉXITO! Archivo recuperado de la DB.");
          } else {
              console.log("¡FALLO! El servicio no pudo crear el archivo.");
          }
      }
